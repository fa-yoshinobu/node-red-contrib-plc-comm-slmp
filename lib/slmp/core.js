...
const PROFILE_UNSUPPORTED_DEVICE_CODES = Object.freeze({
  "melsec:iq-f": new Set(["V", "LTS", "LTC", "LTN", "LSTS", "LSTC", "LSTN", "DX", "DY", "ZR", "RD", "G", "HG"]),
  "melsec:iq-r": new Set(["G", "HG"]),
  "melsec:iq-l": new Set(["G", "HG"]),
  "melsec:mx-f": new Set(["G", "HG"]),
  "melsec:mx-r": new Set(["G", "HG"]),
  "melsec:qcpu": new Set(["LTS", "LTC", "LTN", "LSTS", "LSTC", "LSTN", "LCS", "LCC", "LCN", "LZ", "RD", "G", "HG"]),
  "melsec:lcpu": new Set(["LTS", "LTC", "LTN", "LSTS", "LSTC", "LSTN", "LCS", "LCC", "LCN", "LZ", "RD", "G", "HG"]),
  "melsec:qnu": new Set(["LTS", "LTC", "LTN", "LSTS", "LSTC", "LSTN", "LCS", "LCC", "LCN", "LZ", "RD", "G", "HG"]),
  "melsec:qnudv": new Set(["LTS", "LTC", "LTN", "LSTS", "LSTC", "LSTN", "LCS", "LCC", "LCN", "LZ", "RD", "G", "HG"]),
});
...
function assertDeviceCodeSupportedForPlcProfile(code, plcProfile) {
  const normalizedCode = String(code || "").trim().toUpperCase();
  if (!DEVICE_CODES[normalizedCode]) {
  ...
  if (normalizedProfile && !isDeviceCodeSupportedForPlcProfile(normalizedCode, normalizedProfile.plcProfile)) {
    throw new ValueError(`SLMP device code '${normalizedCode}' is not supported for plcProfile '${normalizedProfile.label}'.`);
  }
}
...
function parseDevice(value, options = {}) {
  const radixProfile = options.addressProfile ?? options.plcProfile ?? options.family ?? null;
  const supportProfile = options.plcProfile ?? options.addressProfile ?? options.family ?? null;
  if (value && typeof value === "object" && typeof value.code === "string" && Number.isInteger(value.number)) {
    const code = value.code.toUpperCase();
    assertDeviceCodeSupportedForPlcProfile(code, supportProfile);
...
function deviceToString(value, options = {}) {
  ...
  const radix = resolveDeviceRadix(ref.code, options.addressProfile ?? options.plcProfile ?? options.family ?? null);
  if (radix === 8) {
