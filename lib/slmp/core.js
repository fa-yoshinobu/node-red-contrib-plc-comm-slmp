...
function parseNumber(value, name, options = {}) {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value) || !Number.isInteger(value)) {
      throw new ValueError(`${name} must be a finite integer`);
    }
    return value;
  }
  ...
  if (!digitPattern.test(normalized)) {
    throw new ValueError(`${name} must be a complete base-${radix} integer: ${value}`);
  }
  const parsed = Number.parseInt(normalized, radix);
  if (!Number.isSafeInteger(parsed)) {
    throw new ValueError(`${name} must be a safe integer: ${value}`);
  }
  return parsed;
}
...
function normalizeTarget(target) {
  ...
  return {
    network: ensureRange(parseNumber(target.network, "target.network"), 0, 0xff, "target.network"),
    station: ensureRange(parseNumber(target.station, "target.station"), 0, 0xff, "target.station"),
    moduleIO: ensureRange(
      parseNumber(moduleIOSource, "target.moduleIO", { base: 16 }),
      0,
      0xffff,
      "target.moduleIO"
    ),
    multidrop: ensureRange(parseNumber(target.multidrop, "target.multidrop"), 0, 0xff, "target.multidrop"),
  };
}
