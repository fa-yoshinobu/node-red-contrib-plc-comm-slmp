...
const PLC_PROFILE_DEFAULTS = Object.freeze({
  ...
  "melsec:mx-r:rj71en71": Object.freeze({ frameType: FrameType.FRAME_4E, plcSeries: PLCSeries.IQR, addressProfile: "melsec:mx-r", rangeProfile: "melsec:mx-r:rj71en71" }),
...
function parseSyntaxInteger(text, name, base) {
  if (typeof text !== "string") {
    throw new ValueError(`${name} must be a base-${base} integer string`);
  }
  const normalized = text.trim();
  const digitPattern = base === 16 ? /^[0-9a-f]+$/i : /^[0-9]+$/;
  if (!digitPattern.test(normalized)) {
    throw new ValueError(`${name} must be a complete base-${base} integer: ${text}`);
  }
  const parsed = Number.parseInt(normalized, base);
  if (!Number.isSafeInteger(parsed)) {
    throw new ValueError(`${name} must be a safe integer: ${text}`);
  }
  return parsed;
}

function requireRuntimeInteger(value, name, min, max) {
  if (typeof value !== "number" || !Number.isFinite(value) || !Number.isSafeInteger(value)) {
    throw new ValueError(`${name} must be a primitive finite safe integer`);
  }
  return ensureRange(value, min, max, name);
}
...
function validateWireDeviceSpan(headAddress, consumedDeviceNumbers, series, label = "device access") {
  const normalizedSeries = normalizePlcSeries(series);
  const maximum = normalizedSeries === PLCSeries.QL ? 0xffffff : 0xffffffff;
  if (!Number.isSafeInteger(headAddress) || headAddress < 0 || headAddress > maximum) {
    throw new ValueError(
      `${label} head address out of ${normalizedSeries} wire namespace (0..${maximum}): ${headAddress}`
    );
  }
  if (!Number.isSafeInteger(consumedDeviceNumbers) || consumedDeviceNumbers < 1) {
    throw new ValueError(
      `${label} consumed device numbers must be a positive safe integer: ${consumedDeviceNumbers}`
    );
  }
  const endAddress = headAddress + consumedDeviceNumbers - 1;
  if (!Number.isSafeInteger(endAddress) || endAddress > maximum) {
    throw new ValueError(
      `${label} device span exceeds ${normalizedSeries} wire namespace: ` +
      `head=${headAddress}, consumedDeviceNumbers=${consumedDeviceNumbers}, ` +
      `end=${endAddress}, maximum=${maximum}`
    );
  }
  return endAddress;
}
...
function normalizeTarget(target) {
  if (target === null || typeof target !== "object" || Array.isArray(target)) {
    throw new ValueError("target must be an object containing network, station, moduleIO, and multidrop");
  }
  const allowedFields = new Set(["network", "station", "moduleIO", "module_io", "multidrop"]);
  for (const field of Object.keys(target)) {
    if (!allowedFields.has(field)) {
      throw new ValueError(`target contains an unexpected field: ${field}`);
    }
  }
  const hasModuleIO = Object.prototype.hasOwnProperty.call(target, "moduleIO");
  const hasModuleIOAlias = Object.prototype.hasOwnProperty.call(target, "module_io");
  if (hasModuleIO && hasModuleIOAlias) {
    throw new ValueError("target must not specify both moduleIO and module_io");
  }
  for (const field of ["network", "station", "multidrop"]) {
    if (!Object.prototype.hasOwnProperty.call(target, field)) {
      throw new ValueError(`target.${field} is required`);
    }
  }
  if (!hasModuleIO && !hasModuleIOAlias) {
    throw new ValueError("target.moduleIO is required");
  }
  const moduleIOSource = hasModuleIO ? target.moduleIO : target.module_io;
  return {
    network: requireRuntimeInteger(target.network, "target.network", 0, 0xff),
    station: requireRuntimeInteger(target.station, "target.station", 0, 0xff),
    moduleIO: requireRuntimeInteger(moduleIOSource, "target.moduleIO", 0, 0xffff),
    multidrop: requireRuntimeInteger(target.multidrop, "target.multidrop", 0, 0xff),
  };
}

function normalizeDecimalInteger(value, name, min, max) {
  if (typeof value === "boolean" || value === null || value === undefined) {
    throw new ValueError(`${name} must be an integer in the range ${min}..${max}`);
  }
  let parsed;
  if (typeof value === "number") {
    ...
    throw new ValueError(`${name} must be a complete decimal integer in the range ${min}..${max}`);
  }
...
function normalizePort(value) {
  return normalizeDecimalInteger(value, "port", 1, 65535);
}

function normalizeTimeout(value) {
  return normalizeDecimalInteger(value, "timeout", 1, 0x7fffffff);
}

function normalizeMonitoringTimer(value) {
  return normalizeDecimalInteger(value, "monitoringTimer", 0, 0xffff);
}
...
function parseRawDevice(value, options = {}) {
...
function requirePublicDeviceProfile(options) {
  if (!options || typeof options !== "object" || Array.isArray(options)) {
    throw new ValueError("parseDevice/deviceToString require options.plcProfile");
  }
  const plcProfile = normalizePlcProfile(options.plcProfile);
  if (!plcProfile) {
    throw new ValueError("parseDevice/deviceToString require options.plcProfile");
  }
  resolveConnectionProfile({ plcProfile });
  return plcProfile;
}

function parseDevice(value, options) {
  const plcProfile = requirePublicDeviceProfile(options);
  if (value && typeof value === "object") {
    const objectProfile = normalizePlcProfile(value.plcProfile);
    if (!objectProfile) {
      throw new ValueError(
        "A semantic device object must include plcProfile. Profile-free {code, number} objects are limited to raw frame encoders."
      );
    }
    if (objectProfile !== plcProfile) {
      throw new ValueError(
        `Device plcProfile '${objectProfile}' does not match requested plcProfile '${plcProfile}'.`
      );
    }
  }
  const ref = parseRawDevice(value, { ...options, plcProfile, addressProfile: plcProfile });
  return Object.freeze({ ...ref, plcProfile });
}
...
function deviceToString(value, options) {
  ...
  return formatRawDevice(ref, plcProfile);
}

function rawDeviceToString(value, options = {}) {
  rejectLegacyFamilyOption(options);
  const ref = parseRawDevice(value, options);
  return formatRawDevice(ref, options.addressProfile ?? options.plcProfile ?? null);
}

function formatRawDevice(ref, radixProfile) {
  const info = DEVICE_CODES[ref.code];
  if (!info) {
    return `${ref.code}${ref.number}`;
  }
  const radix = resolveDeviceRadix(ref.code, radixProfile);
  if (radix === 8) {
...
function normalizeExtensionSpec(extension = {}) {
  if (extension === null || typeof extension !== "object" || Array.isArray(extension)) {
    throw new ValueError("extension must be an object when provided");
  }
  const source = extension;
  const optionalInteger = (camelName, snakeName, defaultValue, max) => {
    const hasCamelName = Object.prototype.hasOwnProperty.call(source, camelName);
    const hasSnakeName = Object.prototype.hasOwnProperty.call(source, snakeName);
    if (hasCamelName && hasSnakeName) {
      throw new ValueError(`extension must not specify both ${camelName} and ${snakeName}`);
    }
    if (!hasCamelName && !hasSnakeName) {
      return defaultValue;
    }
    const value = hasCamelName ? source[camelName] : source[snakeName];
    return requireRuntimeInteger(value, `extension.${camelName}`, 0, max);
  };
  return {
    extensionSpecification: optionalInteger("extensionSpecification", "extension_specification", 0, 0xffff),
    extensionSpecificationModification: optionalInteger(
      "extensionSpecificationModification",
      "extension_specification_modification",
      0,
      0xff
    ),
    deviceModificationIndex: optionalInteger("deviceModificationIndex", "device_modification_index", 0, 0xff),
    deviceModificationFlags: optionalInteger("deviceModificationFlags", "device_modification_flags", 0, 0xff),
    directMemorySpecification: optionalInteger(
...
function requireModificationIndex(index, label) {
  if (!Number.isInteger(index) || index < 0 || index > 0xff) {
    throw new ValueError(`${label} must be an integer in range 0..255`);
  }
  return index;
}
...
class SlmpIndexZ {
  constructor(index) {
    this.index = requireModificationIndex(index, "SlmpIndexZ.index");
    Object.freeze(this);
  }
}
...
class SlmpIndexLz {
  constructor(index) {
    if (!Number.isInteger(index) || index < 0 || index > 1) {
      throw new ValueError("SlmpIndexLz.index must be an integer in range 0..1");
    }
    this.index = index;
    Object.freeze(this);
  }
...
class SlmpExtendedDevice {
  constructor(address, modification = null) {
    if (typeof address !== "string" || address.trim() === "") {
      throw new ValueError("SlmpExtendedDevice.address must be a non-empty device string");
    }
    if (modification !== null &&
        !(modification instanceof SlmpIndexZ) &&
        !(modification instanceof SlmpIndexLz) &&
        !(modification instanceof SlmpIndirect)) {
      throw new ValueError("SlmpExtendedDevice.modification must be SlmpIndexZ, SlmpIndexLz, SlmpIndirect, or null");
    }
...
function parseExtendedDevice(value, options = {}) {
  rejectLegacyFamilyOption(options);
  const parseForContext = options.plcProfile == null ? parseRawDevice : parseDevice;
  if (value && typeof value === "object" && typeof value.code === "string" && Number.isInteger(value.number)) {
    return {
      ref: parseForContext(value, { ...options, allowQualifiedOnly: true }),
  ...
  if (jQualified) {
    const extensionSpecification = ensureRange(
      parseSyntaxInteger(jQualified[1], "extendedDevice.jNetwork", 10),
      0,
      0xff,
      "extendedDevice.jNetwork"
    );
    return {
      ref: parseForContext(jQualified[2], { ...options, allowQualifiedOnly: true }),
  ...
  if (uQualified) {
    const extensionSpecification = ensureRange(
      parseSyntaxInteger(uQualified[1], "extendedDevice.extensionSpecification", 16),
      0,
      0xffff,
      "extendedDevice.extensionSpecification"
    );
    const ref = parseForContext(uQualified[2], { ...options, allowQualifiedOnly: true });
...
function resolveSemanticExtendedDevice(device, options = {}) {
  const semantic = device instanceof SlmpExtendedDevice
    ? device
    : new SlmpExtendedDevice(device);
  const resolved = resolveExtendedDeviceAndExtension(semantic.address, {}, options);
  const modification = semantic.modification;
  if (modification === null) {
    return { ...resolved, address: semantic.address };
  }
  if (resolved.extension.directMemorySpecification === DIRECT_MEMORY_LINK_DIRECT) {
    throw new ValueError("J-qualified link-direct devices do not support Z, LZ, or indirect modification");
  }
  const series = normalizePlcSeries(options.series);
  if (modification instanceof SlmpIndexZ) {
    resolved.extension.deviceModificationIndex = modification.index;
    resolved.extension.deviceModificationFlags = 0x40;
  } else if (modification instanceof SlmpIndexLz) {
    if (series === PLCSeries.QL) {
      throw new ValueError("LZ register modification is not available for Q/L extension subcommands");
    }
    resolved.extension.deviceModificationIndex = modification.index;
    resolved.extension.deviceModificationFlags = 0x80;
  } else {
    resolved.extension.deviceModificationIndex = 0;
    resolved.extension.deviceModificationFlags = 0x08;
  }
  return { ...resolved, address: semantic.address };
}
...
function decode3EResponse(frame, copyFrame = true) {
  const buffer = copyFrame || !Buffer.isBuffer(frame) ? Buffer.from(frame) : frame;
  if (buffer.length < 11) {
...
function decode4EResponse(frame, copyFrame = true) {
  const buffer = copyFrame || !Buffer.isBuffer(frame) ? Buffer.from(frame) : frame;
  if (buffer.length < 15) {
    throw new SlmpError(`response too short: ${buffer.length} bytes`);
  }
  ...
  if (buffer.readUInt16LE(4) !== 0) {
    throw new SlmpError(`unexpected 4E reserved bytes: ${buffer.subarray(4, 6).toString("hex")}`);
  }
...
function decodeResponse(frame, options) {
  ...
  if (normalizedFrameType === FrameType.FRAME_3E) {
    return decode3EResponse(frame, true);
  }
  return decode4EResponse(frame, true);
}

function decodeOwnedResponse(frame, options) {
  if (!Buffer.isBuffer(frame)) {
    throw new TypeError("owned response frame must be a Buffer");
  }
  const normalizedFrameType = normalizeFrameType(options.frameType);
  if (normalizedFrameType === FrameType.FRAME_3E) {
    return decode3EResponse(frame, false);
  }
  return decode4EResponse(frame, false);
}
...
function encodeDeviceSpec(device, options) {
  const ref = parseRawDevice(device, options);
  const info = DEVICE_CODES[ref.code];
  const normalizedSeries = normalizePlcSeries(options.series);
  if (!info) {
    throw new ValueError(`Unknown SLMP device code '${ref.code}'`);
  }
  // PROFILE_RANGE_NOT_A_TRANSPORT_GUARD: Profile device ranges are
  // application-layer metadata. Transport validation covers syntax,
  // supported device families, and only the selected wire format width.
  if (normalizedSeries === PLCSeries.QL) {
...
function encodeResolvedExtendedDeviceSpec(ref, options) {
  const prepared = prepareResolvedExtendedDeviceSpec(ref, options);
  const buffer = Buffer.alloc(prepared.length);
  writePreparedExtendedDeviceSpec(buffer, 0, prepared);
  return buffer;
}

function prepareResolvedExtendedDeviceSpec(ref, options) {
  ...
  const series = normalizePlcSeries(options.series);
  ensureRange(ref.number, 0, series === PLCSeries.QL ? 0xffffff : 0xffffffff, "device.number");
  return Object.freeze({
    code: info.code,
    extension,
    length: series === PLCSeries.QL ? 11 : 13,
    linkDirect: false,
    number: ref.number,
    series,
  });
...
function writePreparedExtendedDeviceSpec(buffer, offset, prepared) {
  const extension = prepared.extension;
  if (prepared.linkDirect) {
    buffer.fill(0, offset, offset + prepared.length);
    buffer.writeUIntLE(prepared.number, offset + 2, 3);
    buffer.writeUInt8(prepared.code & 0xff, offset + 5);
    buffer.writeUInt8(extension.extensionSpecification & 0xff, offset + 8);
    buffer.writeUInt8(DIRECT_MEMORY_LINK_DIRECT, offset + 10);
    return offset + prepared.length;
  }

  buffer.writeUInt8(extension.deviceModificationIndex, offset);
  buffer.writeUInt8(extension.deviceModificationFlags, offset + 1);
  let cursor = offset + 2;
  if (prepared.series === PLCSeries.QL) {
    buffer.writeUIntLE(prepared.number, cursor, 3);
    buffer.writeUInt8(prepared.code & 0xff, cursor + 3);
    cursor += 4;
  } else {
    buffer.writeUInt32LE(prepared.number, cursor);
    buffer.writeUInt16LE(prepared.code, cursor + 4);
    cursor += 6;
  }
  buffer.writeUInt8(extension.extensionSpecificationModification, cursor);
  buffer.writeUInt8(0, cursor + 1);
  buffer.writeUInt16LE(extension.extensionSpecification, cursor + 2);
  buffer.writeUInt8(extension.directMemorySpecification, cursor + 4);
  return offset + prepared.length;
}
...
function numberToBufferCore(value, size) {
  ...
  if (typeof value !== "number" || !Number.isInteger(value)) {
    throw new ValueError(`value must be an integer: ${String(value)}`);
  }
  if (size === 2) {
    ensureRange(value, 0, 0xffff, "value");
    buffer.writeUInt16LE(value, 0);
    return buffer;
  }
  if (size === 4) {
    ensureRange(value, 0, 0xffffffff, "value");
...
function packBitValues(values) {
  const bits = Array.from(values, (value) => {
    if (typeof value === "boolean") return value ? 1 : 0;
    throw new ValueError(`bit value must be boolean: ${String(value)}`);
  });
  const packed = [];
...
function unpackBitValues(data, count) {
  const buffer = Buffer.isBuffer(data) ? data : Buffer.from(data);
  const expectedLength = Math.ceil(count / 2);
  if (buffer.length !== expectedLength) {
    throw new SlmpError(`bit data length mismatch: expected=${expectedLength}, actual=${buffer.length}`);
  }
  const values = [];
  for (const byte of buffer) {
    const high = (byte >> 4) & 0x0f;
    const low = byte & 0x0f;
    if (high > 1 || (values.length + 1 < count && low > 1)) {
      throw new SlmpError(`bit data contains an invalid nibble: 0x${byte.toString(16).padStart(2, "0")}`);
    }
    values.push(high === 1);
...
Object.defineProperties(module.exports, {
  _decodeOwnedResponse: { value: decodeOwnedResponse },
  _encodeExtendedDeviceSpec: { value: encodeExtendedDeviceSpec },
  _encodeResolvedExtendedDeviceSpec: { value: encodeResolvedExtendedDeviceSpec },
  _prepareResolvedExtendedDeviceSpec: { value: prepareResolvedExtendedDeviceSpec },
  _resolveSemanticExtendedDevice: { value: resolveSemanticExtendedDevice },
  _writePreparedExtendedDeviceSpec: { value: writePreparedExtendedDeviceSpec },
  _validateWireDeviceSpan: { value: validateWireDeviceSpan },
});
