...
const PROFILE_UNSUPPORTED_DEVICE_CODES = Object.freeze({
  "iq-f": new Set(["V", "LTS", "LTC", "LTN", "LSTS", "LSTC", "LSTN", "DX", "DY", "ZR", "RD", "G", "HG"]),
  "iq-r": new Set(["G", "HG"]),
  "iq-l": new Set(["G", "HG"]),
  "mx-f": new Set(["G", "HG"]),
  "mx-r": new Set(["G", "HG"]),
  qcpu: new Set(["LTS", "LTC", "LTN", "LSTS", "LSTC", "LSTN", "LCS", "LCC", "LCN", "LZ", "RD", "G", "HG"]),
  lcpu: new Set(["LTS", "LTC", "LTN", "LSTS", "LSTC", "LSTN", "LCS", "LCC", "LCN", "LZ", "RD", "G", "HG"]),
  qnu: new Set(["LTS", "LTC", "LTN", "LSTS", "LSTC", "LSTN", "LCS", "LCC", "LCN", "LZ", "RD", "G", "HG"]),
  qnudv: new Set(["LTS", "LTC", "LTN", "LSTS", "LSTC", "LSTN", "LCS", "LCC", "LCN", "LZ", "RD", "G", "HG"]),
});
const PUBLIC_HIGH_LEVEL_UNSUPPORTED_DEVICE_CODES = new Set(["G", "HG"]);
const PLC_PROFILE_DEFAULTS = Object.freeze({
  "melsec:iq-f": Object.freeze({ frameType: FrameType.FRAME_3E, plcSeries: PLCSeries.QL, deviceFamily: "iq-f" }),
  "melsec:iq-r": Object.freeze({ frameType: FrameType.FRAME_4E, plcSeries: PLCSeries.IQR, deviceFamily: "iq-r" }),
  "melsec:iq-l": Object.freeze({ frameType: FrameType.FRAME_4E, plcSeries: PLCSeries.IQR, deviceFamily: "iq-r" }),
  "melsec:mx-f": Object.freeze({ frameType: FrameType.FRAME_4E, plcSeries: PLCSeries.IQR, deviceFamily: "mx-f" }),
  "melsec:mx-r": Object.freeze({ frameType: FrameType.FRAME_4E, plcSeries: PLCSeries.IQR, deviceFamily: "mx-r" }),
  "melsec:qcpu": Object.freeze({ frameType: FrameType.FRAME_3E, plcSeries: PLCSeries.QL, deviceFamily: "qcpu" }),
  "melsec:lcpu": Object.freeze({ frameType: FrameType.FRAME_3E, plcSeries: PLCSeries.QL, deviceFamily: "lcpu" }),
  "melsec:qnu": Object.freeze({ frameType: FrameType.FRAME_3E, plcSeries: PLCSeries.QL, deviceFamily: "qnu" }),
  "melsec:qnudv": Object.freeze({ frameType: FrameType.FRAME_3E, plcSeries: PLCSeries.QL, deviceFamily: "qnudv" }),
});
const DEVICE_FAMILY_LABELS = Object.freeze({
  "iq-f": "melsec:iq-f",
  "iq-r": "melsec:iq-r",
  "iq-l": "melsec:iq-l",
  "mx-f": "melsec:mx-f",
...
function normalizeDeviceFamilyOrProfile(value) {
  if (value === undefined || value === null || value === "") {
    return null;
  }
  const normalized = String(value).trim().toLowerCase();
  if (PLC_PROFILES.has(normalized)) {
    return {
      deviceFamily: PLC_PROFILE_DEFAULTS[normalized].deviceFamily,
      label: normalized,
    };
  }
  if (Object.prototype.hasOwnProperty.call(DEVICE_FAMILY_LABELS, normalized)) {
    return {
      deviceFamily: normalized,
      label: DEVICE_FAMILY_LABELS[normalized],
    };
  }
  throw new ValueError(`Unsupported plcProfile/deviceFamily: ${value}`);
}
...
function resolveConnectionProfile({ plcProfile, plcSeries, frameType } = {}, options = {}) {
  ...
  if (normalizedProfile) {
    ...
    return {
      ...
      deviceFamily: PLC_PROFILE_DEFAULTS[normalizedProfile].deviceFamily,
    };
  }
...
function resolveDeviceRadix(code, family) {
  const normalizedFamily = normalizeDeviceFamilyOrProfile(family);
  if (normalizedFamily && normalizedFamily.deviceFamily === "iq-f" && IQF_OCTAL_DEVICE_CODES.has(code)) {
    return 8;
  }
  return DEVICE_CODES[code].radix;
}

function isDeviceCodeSupportedForFamily(code, family) {
  ...
  const unsupported = PROFILE_UNSUPPORTED_DEVICE_CODES[normalizedFamily.deviceFamily];
  return !(unsupported && unsupported.has(normalizedCode));
}

function assertDeviceCodeSupportedForFamily(code, family) {
  const normalizedCode = String(code || "").trim().toUpperCase();
  if (!DEVICE_CODES[normalizedCode]) {
  ...
  if (PUBLIC_HIGH_LEVEL_UNSUPPORTED_DEVICE_CODES.has(normalizedCode)) {
    if (normalizedFamily) {
      throw new ValueError(`SLMP device code '${normalizedCode}' is not supported for plcProfile '${normalizedFamily.label}'.`);
    }
    throw new ValueError(
      `SLMP device code '${normalizedCode}' is not supported in the Node-RED public high-level surface.`
    );
  }
  if (normalizedFamily && !isDeviceCodeSupportedForFamily(normalizedCode, normalizedFamily.deviceFamily)) {
    throw new ValueError(`SLMP device code '${normalizedCode}' is not supported for plcProfile '${normalizedFamily.label}'.`);
  }
}
...
function parseDevice(value, options = {}) {
  const radixFamily = options.family ?? options.plcProfile ?? null;
  const supportFamily = options.plcProfile ?? options.family ?? null;
  if (value && typeof value === "object" && typeof value.code === "string" && Number.isInteger(value.number)) {
    const code = value.code.toUpperCase();
    assertDeviceCodeSupportedForFamily(code, supportFamily);
  ...
  assertDeviceCodeSupportedForFamily(code, supportFamily);
  const radix = resolveDeviceRadix(code, radixFamily);
...
function deviceToString(value, options = {}) {
  ...
  const radix = resolveDeviceRadix(ref.code, options.family ?? options.plcProfile ?? null);
  if (radix === 8) {
