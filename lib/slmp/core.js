...
const FAMILY_UNSUPPORTED_DEVICE_CODES = Object.freeze({
...
const PLC_FAMILY_DEFAULTS = Object.freeze({
  "iq-f": Object.freeze({ frameType: FrameType.FRAME_3E, plcSeries: PLCSeries.QL, deviceFamily: "iq-f" }),
  "iq-r": Object.freeze({ frameType: FrameType.FRAME_4E, plcSeries: PLCSeries.IQR, deviceFamily: "iq-r" }),
  "iq-l": Object.freeze({ frameType: FrameType.FRAME_4E, plcSeries: PLCSeries.IQR, deviceFamily: "iq-r" }),
  "mx-f": Object.freeze({ frameType: FrameType.FRAME_4E, plcSeries: PLCSeries.IQR, deviceFamily: "mx-f" }),
  "mx-r": Object.freeze({ frameType: FrameType.FRAME_4E, plcSeries: PLCSeries.IQR, deviceFamily: "mx-r" }),
  qcpu: Object.freeze({ frameType: FrameType.FRAME_3E, plcSeries: PLCSeries.QL, deviceFamily: "qcpu" }),
  lcpu: Object.freeze({ frameType: FrameType.FRAME_3E, plcSeries: PLCSeries.QL, deviceFamily: "lcpu" }),
  qnu: Object.freeze({ frameType: FrameType.FRAME_3E, plcSeries: PLCSeries.QL, deviceFamily: "qnu" }),
  qnudv: Object.freeze({ frameType: FrameType.FRAME_3E, plcSeries: PLCSeries.QL, deviceFamily: "qnudv" }),
});
...
function resolveConnectionProfile({ plcFamily, plcSeries, frameType } = {}, options = {}) {
  ...
  if (normalizedFamily) {
    if (plcSeries !== undefined || frameType !== undefined) {
      throw new ValueError(
        "plcFamily already determines frameType, plcSeries, and address handling. Do not also pass plcSeries or frameType."
      );
    }
    return {
  ...
  if (!allowManualProfile) {
    throw new ValueError(
      "plcFamily is required for the standard client profile. The normal route derives frameType, plcSeries, and address handling from that explicit family."
    );
  }
  return {
    plcFamily: null,
...
function isDeviceCodeSupportedForFamily(code, family) {
  ...
  const unsupported = FAMILY_UNSUPPORTED_DEVICE_CODES[normalizedFamily];
...
function assertDeviceCodeSupportedForFamily(code, family) {
  ...
  const normalizedFamily = normalizePlcFamily(family);
  if (normalizedFamily && !isDeviceCodeSupportedForFamily(normalizedCode, normalizedFamily)) {
    throw new ValueError(`SLMP device code '${normalizedCode}' is not supported for plcFamily '${normalizedFamily}'.`);
  }
}
...
function parseDevice(value, options = {}) {
  const radixFamily = options.family ?? options.plcFamily ?? null;
  const supportFamily = options.plcFamily ?? options.family ?? null;
