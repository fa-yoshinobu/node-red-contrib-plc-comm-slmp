...
const { getEndCodeName, isRemotePasswordEndCode } = require("./error-codes");
...
class SlmpTimeoutError extends SlmpError {
  constructor(message, options = {}) {
    super(message, options);
    this.name = "SlmpTimeoutError";
    this.code = "SLMP_TIMEOUT";
  }
}

class SlmpNotConnectedError extends SlmpError {
  constructor(message = "SLMP transport is not connected", options = {}) {
    super(message, options);
    this.name = "SlmpNotConnectedError";
    this.code = "SLMP_NOT_CONNECTED";
  }
}

class SlmpClosedError extends SlmpError {
  constructor(message = "SLMP transport was closed", options = {}) {
    super(message, options);
    this.name = "SlmpClosedError";
    this.code = "SLMP_CLOSED";
  }
}

class SlmpOperationOutcomeUnknownError extends SlmpError {
  constructor(message, reason, options = {}) {
    super(message, options);
    this.name = "SlmpOperationOutcomeUnknownError";
    this.code = "SLMP_OPERATION_OUTCOME_UNKNOWN";
    this.reason = reason;
  }
}
...
class SlmpProfileFeatureError extends Error {
  constructor({ profileId, featureKey, state, evidence, disableHint } = {}) {
    const hint = disableHint || "Use a profile and feature combination whose capability state permits this request.";
