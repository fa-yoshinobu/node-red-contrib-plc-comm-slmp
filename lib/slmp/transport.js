...
class SlmpTransport {
  ...
  async sendAndReceive(frame, serial, expectedTarget = null) {
    if (this.transportType === "tcp") {
      if (!this._tcpSocket) {
        throw new SlmpError("TCP socket is not connected");
      }
      ...
      const responsePromise = this.awaitTcpFrame(serial, expectedTarget);
      try {
        await new Promise((resolve, reject) => {
          this._tcpSocket.write(frame, (error) => {
            if (error) {
    ...
    return this.sendUdp(frame, serial, expectedTarget);
  }
      ...
      const finalizeResolve = () => {
        ...
        try {
          ...
          try {
            ...
            // Preserve the socket-option failure as the connection diagnosis.
          }
          reject(new SlmpError(`TCP keepalive configuration failed: ${error.message}`, { cause: error }));
          return;
        }
        settled = true;
        clearTimeout(timeoutHandle);
  ...
  awaitTcpFrame(serial, expectedTarget = null) {
    if (this.frameType === FrameType.FRAME_4E) {
  ...
  sendUdp(frame, serial, expectedTarget = null) {
    return new Promise((resolve, reject) => {
      if (!this._udpSocket) {
        reject(new SlmpError("UDP socket is not connected"));
        return;
      ...
      const timeoutHandle = setTimeout(() => {
        ...
        this._failUdpAndClose(new SlmpError("UDP communication timeout"), socket);
      }, this.timeout);
