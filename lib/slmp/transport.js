...
class TcpFrameAccumulator {
  constructor(frameType) {
    ...
    this._maximumFrameLength = frameType === FrameType.FRAME_4E ? 13 + 0xffff : 9 + 0xffff;
  ...
  append(chunk) {
    ...
    if (source.length > this._maximumFrameLength - this.length) {
      throw new SlmpError("response frame exceeds the protocol length limit");
    }
    this._ensureWritable(source.length);
    source.copy(this._buffer, this._end);
    this._end += source.length;
    this._copiedBytes += source.length;
  }

  tryExtract() {
    const is4E = this.frameType === FrameType.FRAME_4E;
    const headerSize = is4E ? 13 : 9;
    const expectedSubheader = is4E ? 0x00d4 : 0x00d0;
    if (this.length >= 2 && this._buffer.readUInt16LE(this._start) !== expectedSubheader) {
      throw new SlmpError("unexpected response frame type");
    }
    if (this.length < headerSize) return null;

    const responseDataLength = this._buffer.readUInt16LE(this._start + headerSize - 2);
    const totalLength = headerSize + responseDataLength;
    if (this.length < totalLength) return null;

    // The returned frame has one stable owner. Later socket data may compact or
    // overwrite the accumulator without changing an already delivered frame.
    const frame = Buffer.allocUnsafe(totalLength);
    this._buffer.copy(frame, 0, this._start, this._start + totalLength);
    this._copiedBytes += totalLength;
    this._start += totalLength;
  ...
  _ensureWritable(additionalLength) {
    if (this._buffer.length - this._end >= additionalLength) return;

    const bufferedLength = this.length;
    if (this._start > 0 && this._buffer.length - bufferedLength >= additionalLength) {
      this._buffer.copy(this._buffer, 0, this._start, this._end);
      this._copiedBytes += bufferedLength;
      this._start = 0;
      this._end = bufferedLength;
      return;
    }

    const requiredLength = bufferedLength + additionalLength;
    let capacity = Math.min(this._maximumFrameLength, Math.max(256, this._buffer.length || 0));
    while (capacity < requiredLength) {
      capacity = Math.min(this._maximumFrameLength, capacity * 2);
    }
    const replacement = Buffer.allocUnsafe(capacity);
    if (bufferedLength > 0) {
      this._buffer.copy(replacement, 0, this._start, this._end);
      this._copiedBytes += bufferedLength;
    }
...
class SlmpTransport {
  constructor(options) {
    ...
    this._tcpAccumulator = new TcpFrameAccumulator(this.frameType);
  ...
  handleTcpData(chunk, socket = null) {
    ...
    try {
      this._tcpAccumulator.append(chunk);
    } catch (error) {
    ...
    while (true) {
      let frame;
      try {
        frame = this._tcpAccumulator.tryExtract();
      } catch (error) {
        this._failTcpAndDestroy(asProtocolError(error));
        return;
      }
  ...
  sendUdp(frame, serial, expectedTarget = null, deadline = null) {
    return new Promise((resolve, reject) => {
      ...
      socket.send(frame, (error) => {
        ...
        const pending = this.frameType === FrameType.FRAME_4E
          ? this._udpPendingBySerial.get(serial)
          : this._udpPending;
        if (!pending || pending.socket !== socket || pending.sendComplete) {
          return;
        }
        if (error) {
        ...
        this._recordSend(frame);
        if (pending.provisionalResponse) {
          this._recordReceive(pending.provisionalResponse);
          this._resolveUdpPending(serial, pending, pending.provisionalResponse);
        }
      });
    });
  }
  ...
  handleUdpMessage(message, socket = this._udpSocket) {
    ...
    if (this.frameType === FrameType.FRAME_4E) {
      ...
      if (!pending.sendComplete) {
        if (!pending.provisionalResponse) pending.provisionalResponse = Buffer.from(message);
        return;
      }
      this._recordReceive(message);
      this._resolveUdpPending(metadata.serial, pending, message);
      return;
    }
    ...
    this._recordReceive(message);
    this._resolveUdpPending(0, pending, message);
  }

  _resolveUdpPending(serial, pending, message) {
    if (this.frameType === FrameType.FRAME_4E) {
      if (this._udpPendingBySerial.get(serial) !== pending) return;
      this._udpPendingBySerial.delete(serial);
    } else {
      if (this._udpPending !== pending) return;
      this._udpPending = null;
    }
    pending.provisionalResponse = null;
    clearTimeout(pending.timeoutHandle);
  ...
  handleUdpFailure(error, socket = this._udpSocket) {
    ...
    const classified = error instanceof SlmpError
      ? error
      : new SlmpError(`UDP transport failure: ${error.message}`, { cause: error });
    this._failUdpAndClose(classified, socket);
  }
