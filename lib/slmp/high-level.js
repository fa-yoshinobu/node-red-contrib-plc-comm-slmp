...
function canonicalizeDtype(dtype) {
  const key = String(dtype ?? "").trim().toUpperCase();
  if (key === "STRING") {
    return "STR";
  }
  return key === "I" ? "S" : key;
...
function parseAddress(address) {
  ...
  if (core.includes(":")) {
    ...
    return { base: base.trim(), dtype: canonicalDtype, bitIndex: null, count, hasCount, explicitDtype: true };
  }
  if (core.includes(".")) {
...
function formatParsedAddress(parsed, options = {}) {
  ...
  let text = base;
  const dtype = requireDtype(parsed.dtype);
...
function resolveEntryDtype(address, device, parsed) {
  const normalized = requireDtype(normalizeDtypeForDevice(device, parsed.dtype));
  return normalized;
}
...
async function readRandomMaps(client, plan, options = {}) {
  ...
  while (wordIndex < words.length || dwordIndex < dwords.length) {
    const wordTake = Math.min(words.length - wordIndex, RANDOM_READ_BATCH_LIMIT);
    const dwordTake = Math.min(dwords.length - dwordIndex, RANDOM_READ_BATCH_LIMIT - wordTake);
    const wordChunk = words.slice(wordIndex, wordIndex + wordTake);
    const dwordChunk = dwords.slice(dwordIndex, dwordIndex + dwordTake);
    wordIndex += wordTake;
    dwordIndex += dwordTake;
    tasks.push(
...
async function executeRandomWrites(client, entries, options = {}) {
  ...
  while (wordIndex < wordValues.length || dwordIndex < dwordValues.length) {
    let budget = RANDOM_WRITE_WEIGHT_LIMIT;
    const wordChunk = [];
    while (wordIndex < wordValues.length && budget >= RANDOM_WRITE_WORD_WEIGHT) {
      wordChunk.push(wordValues[wordIndex]);
      wordIndex += 1;
      budget -= RANDOM_WRITE_WORD_WEIGHT;
    }
    const dwordChunk = [];
    while (dwordIndex < dwordValues.length && budget >= RANDOM_WRITE_DWORD_WEIGHT) {
      dwordChunk.push(dwordValues[dwordIndex]);
      dwordIndex += 1;
      budget -= RANDOM_WRITE_DWORD_WEIGHT;
    }
...
async function executeWordWriteCluster(client, cluster, options = {}) {
  ...
  for (const entry of entries) {
    ...
    if (entry.dtype === "BIT_IN_WORD") {
      ...
      const bitIndex = requireBitInWordIndex(entry.address, entry.bitIndex);
      if (entry.value) {
        current |= 1 << bitIndex;
      } else {
        current &= ~(1 << bitIndex);
...
async function executeLongCurrentWrites(client, entries, options = {}) {
  ...
  for (const chunk of chunkArray(dwordValues, Math.floor(RANDOM_WRITE_WEIGHT_LIMIT / RANDOM_WRITE_DWORD_WEIGHT))) {
    if (chunk.length === 0) {
      continue;
    }
...
async function executeRandomBitWrites(client, entries, options = {}) {
  ...
  for (const chunk of chunkArray(bitValues, RANDOM_WRITE_BIT_BATCH_LIMIT)) {
    if (chunk.length === 0) {
      continue;
    }
...
function deviceToStringWithContext(device, options = {}, client = null) {
  return deviceToString(device, { addressProfile: resolveAddressProfile(options, client) });
}
