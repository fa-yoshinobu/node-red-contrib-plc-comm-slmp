...
function requirePositiveSafeIntegerCount(value) {
  if (typeof value !== "number" || !Number.isSafeInteger(value) || value <= 0) {
    throw new ValueError("parsed address count must be a native positive safe-integer number");
  }
  return value;
}
...
function formatParsedAddress(parsed, options) {
  ...
  text += `:${dtype}`;
  if (parsed.hasCount) {
    text += `,${requirePositiveSafeIntegerCount(parsed.count)}`;
  }
  return text;
}
...
function randomWriteWordLimits(client) {
  ...
  return {
    count: profileLimit && Number.isInteger(profileLimit.max)
      ? profileLimit.max
      : iqrFallback ? 80 : 160,
    weighted: profileLimit && Number.isInteger(profileLimit.weighted_max)
