...
const LONG_STATE_RANDOM_BIT_CODES = new Set(["LTC", "LTS", "LSTC", "LSTS"]);
...
function parseDeviceWithContext(device, options = {}, client = null) {
  ...
  const ref = parseDevice(device, { family });
  return requireExplicitDeviceFamilyForXY(device, resolveExplicitPlcFamily(options, client), ref);
}
