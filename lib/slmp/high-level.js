...
async function executeWordWriteCluster(client, cluster, options = {}) {
  ...
  const words = needsRead
    ? Array.from(
        await client.readDevices(makeDeviceRef(cluster.code, cluster.start, cluster.plcProfile), wordCount, { ...options, bitUnit: false })
      )
    : new Array(wordCount);
  const fullAssignments = new Set();
  const bitAssignments = new Set();

  for (const entry of entries) {
    const offset = entry.device.number - cluster.start;
    if (entry.dtype === "BIT_IN_WORD") {
      if (fullAssignments.has(offset)) {
        throw new ValueError(`writeNamed contains overlapping destination ${cluster.code}${cluster.start + offset}`);
      }
      bitAssignments.add(offset);
    ...
    for (let index = 0; index < encoded.length; index += 1) {
      const slot = offset + index;
      if (fullAssignments.has(slot) || bitAssignments.has(slot)) {
        throw new ValueError(`writeNamed contains overlapping destination ${cluster.code}${cluster.start + slot}`);
      }
      fullAssignments.add(slot);
      words[slot] = encoded[index];
    }
  }

  requireFullySpecifiedCluster(cluster, words, "word");
  await client.writeDevices(makeDeviceRef(cluster.code, cluster.start, cluster.plcProfile), words, { ...options, bitUnit: false });
}
...
async function executeWordWriteClusters(client, clusters, options = {}) {
  ...
  await client.writeBlock({ wordBlocks, ...options });
}
