...
class SlmpClient {
  constructor(options) {
    ...
    this.defaultTarget = normalizeTarget(hasDefaultTarget ? source.defaultTarget : source.target);
  ...
  _ensureProfileFeatureAllowed(featureKey) {
    ensureProfileFeatureAllowed(this.plcProfile, featureKey, this._strictProfile);
  }
  ...
  async _sendRemotePasswordCommand(command, password, options = {}) {
    ...
    await this._request(command, 0x0000, encodePassword(password, normalizedSeries), requestOptions);
  }
