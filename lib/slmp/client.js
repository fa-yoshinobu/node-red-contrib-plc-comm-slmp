...
class SlmpClient {
  constructor(options) {
    ...
    this.deviceRangeFamily = profile.rangeFamily;
  ...
  _parseDevice(device) {
    const ref = parseDevice(device, { family: this.deviceFamily });
  ...
  async readDeviceRangeCatalog(options = {}) {
    if (!this.deviceRangeFamily) {
      throw new ValueError("readDeviceRangeCatalog requires explicit plcFamily for the high-level client.");
    }
    return readDeviceRangeCatalogForFamily(this, this.deviceRangeFamily, options);
  }
...
function normalizeItems(values, deviceFamily, plcFamily) {
  if (Array.isArray(values)) {
    return values.map(([device, value]) => [
      requireExplicitDeviceFamilyForXY(device, plcFamily ?? deviceFamily, parseDevice(device, { family: deviceFamily })),
      value,
    ]);
  }
...
function normalizeBlockItems(values, label, deviceFamily, plcFamily) {
  return Array.from(values || [], (item) => {
    if (Array.isArray(item)) {
      ...
      return {
        device: requireExplicitDeviceFamilyForXY(device, plcFamily ?? deviceFamily, parseDevice(device, { family: deviceFamily })),
    ...
    return {
      device: requireExplicitDeviceFamilyForXY(
        ...
        parseDevice(item.device, { family: deviceFamily })
      ),
