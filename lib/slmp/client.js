...
class SlmpClient {
  ...
  _request(command, subcommand, data, options = {}) {
    ...
    return this._dispatchPreparedRequest(command, subcommand, requestData, requestOptions, null);
  }

  _requestDecoded(command, subcommand, data, options, responseDecoder) {
    if (typeof responseDecoder !== "function") {
      throw new ValueError("responseDecoder must be a function");
    }
    if (this._request !== SlmpClient.prototype._request ||
        this._requestInternal !== SlmpClient.prototype._requestInternal) {
      return Promise.resolve(this._request(command, subcommand, data, options)).then(responseDecoder);
    }
    const { requestData, requestOptions } = this._prepareRequest(command, subcommand, data, options);
    return this._dispatchPreparedRequest(command, subcommand, requestData, requestOptions, responseDecoder);
  }

  _dispatchPreparedRequest(command, subcommand, requestData, requestOptions, responseDecoder) {
  ...
  async readArrayLabels(points, options = {}) {
    ...
    return this._requestDecoded(
      ...
      options,
      (response) => parseArrayLabelReadResponse(response.data, normalized),
    );
  }
  ...
  async readRandomLabels(labels, options = {}) {
    ...
    return this._requestDecoded(
      ...
      options,
      (response) => parseRandomLabelReadResponse(response.data, normalized.length),
    );
  }
  ...
  async _requestInternal(
    ...
    return responseDecoder === null ? response : responseDecoder(response);
  }
