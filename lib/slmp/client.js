...
function validateBlockReadDevices(wordBlocks, bitBlocks) {
  const bitDeviceInWordBlocks = wordBlocks.find(
    (block) => DEVICE_CODES[block.device.code]?.unit !== DeviceUnit.WORD
  );
  if (bitDeviceInWordBlocks) {
    throw new ValueError(
      `Read Block (0x0406) wordBlocks requires a word device; received ${bitDeviceInWordBlocks.device.code}`
    );
  }
  const wordDeviceInBitBlocks = bitBlocks.find(
    (block) => DEVICE_CODES[block.device.code]?.unit !== DeviceUnit.BIT
  );
  if (wordDeviceInBitBlocks) {
    throw new ValueError(
      `Read Block (0x0406) bitBlocks requires a bit device; received ${wordDeviceInBitBlocks.device.code}`
    );
  }
...
function validateBlockWriteDevices(wordBlocks, bitBlocks, plcProfile) {
  ...
  if (bitDeviceInWordBlocks) {
    throw new ValueError(
      `Write Block (0x1406) wordBlocks requires a word device; received ${bitDeviceInWordBlocks.device.code}`
    );
  }
  const wordDeviceInBitBlocks = bitBlocks.find(
    (block) => DEVICE_CODES[block.device.code]?.unit !== DeviceUnit.BIT
  ...
  if (wordDeviceInBitBlocks) {
    throw new ValueError(
      `Write Block (0x1406) bitBlocks requires a bit device; received ${wordDeviceInBitBlocks.device.code}`
    );
  }
...
class SlmpClient {
  ...
  async _unlockRemotePasswordIfConfigured(deadline = null) {
  ...
  _prepareRequest(command, subcommand, data, options = {}) {
    ...
    const requestOptions = {
      ...
      raiseOnError: Object.prototype.hasOwnProperty.call(options, "raiseOnError")
        ...
        : this.raiseOnError,
      stateChanging: classifyStateChangingCommand(command, options),
    };
  ...
  _request(command, subcommand, data, options = {}) {
    const { requestData, requestOptions } = this._prepareRequest(command, subcommand, data, options);
    ...
    if (context?.client === this) {
      ...
      return this._requestInternal(command, subcommand, requestData, requestOptions);
    }
  ...
  _enqueueOperation(operation) {
    ...
    const task = this._requestChain.then(async () => {
      this._queuedOperationCount -= 1;
  ...
  _runExclusive(operation) {
    ...
    return this._enqueueOperation(() => {
      const context = { client: this, generation: this._clientGeneration };
  ...
  async readRandomExt({ wordDevices = [], dwordDevices = [], series, ...requestOptions } = {}) {
    ...
    words.forEach((entry) => {
      ...
      parts.push(encodeResolvedExtendedDeviceSpec(entry.device, { series: extendedSeries, extension: entry.extension }));
    });
  ...
  async writeRandomBitsExt({ bitValues = [], series, ...requestOptions } = {}) {
    ...
    bits.forEach((entry) => {
      ...
      if (extendedSeries === PLCSeries.IQR) {
        parts.push(numberToBuffer(state, 2));
      } else {
        parts.push(Buffer.from([state]));
      }
  ...
  async readArrayLabels(points, options = {}) {
    ...
    return parseArrayLabelReadResponse(response.data, normalized);
  }
  ...
  async _sendManagedRemotePasswordCommand(command, password, deadline = null) {
  ...
  async _requestInternal(command, subcommand, data, options, internalContext = null, inheritedDeadline = null) {
    rejectProfileDerivedOverrides(options, "request");
    const requestData = Buffer.from(data || Buffer.alloc(0));
    validateRequestPayloadLength(
      requestData.length,
      maxRequestPayloadLength(this.transportType, this.frameType)
    );
    ...
    const stateChanging = Object.prototype.hasOwnProperty.call(options, "stateChanging")
      ? options.stateChanging
      : classifyStateChangingCommand(command, options);
    if (options.expectResponse === false) {
      await this._sendOnly(frame, options, internalContext, deadline, stateChanging);
  ...
  async _sendOnly(frame, options = {}, internalContext = null, deadline = null, stateChanging = false) {
    if (internalContext !== MANAGED_REMOTE_PASSWORD_COMMAND) {
      ...
      await this._unlockRemotePasswordIfConfigured(deadline);
    }
...
function validateNoBlockWriteOverlap(wordBlocks, bitBlocks) {
  const blocks = [
    ...wordBlocks.map((block) => ({ ...block, destinationSpan: block.values.length })),
    ...bitBlocks.map((block) => ({ ...block, destinationSpan: block.values.length * 16 })),
  ];
...
function asOutcomeUnknown(error) {
  if (error instanceof SlmpOperationOutcomeUnknownError) return error;
  return new SlmpOperationOutcomeUnknownError(
    `SLMP state-changing operation outcome is unknown after send (${outcomeUnknownReason(error)})`,
    outcomeUnknownReason(error),
    { cause: error },
  );
}
...
function parseArrayLabelReadResponse(data, requestedPoints) {
  const payload = Buffer.from(data || Buffer.alloc(0));
  if (payload.length < 2) {
  ...
  if (count !== requestedPoints.length) {
    throw new SlmpError(`array label read point count mismatch: expected=${requestedPoints.length}, actual=${count}`);
  }
  let offset = 2;
  const results = [];
  for (let index = 0; index < count; index += 1) {
    ...
    if (![0, 1].includes(unitSpecification)) {
      throw new SlmpError(`array label read response has invalid unitSpecification: ${unitSpecification}`);
    }
    if (arrayDataLength === 0) {
      throw new SlmpError("array label read response has zero arrayDataLength");
    }
...
function parseRandomLabelReadResponse(data, expectedPoints) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (readDataLength === 0 || readDataLength % 2 !== 0) {
      throw new SlmpError(`label random read response data length must be positive and even: ${readDataLength}`);
    }
