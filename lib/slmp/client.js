...
function validateRandomReadDevices(wordDevices, dwordDevices, options = {}) {
  if (!options.allowQualifiedOnly && [...wordDevices, ...dwordDevices].some((device) => QUALIFIED_ONLY_CODES.has(device.code))) {
    throw new ValueError("Read Random (0x0403) does not support standalone G/HG. Use U-qualified extended access.");
  }
...
class SlmpClient {
  constructor(options) {
    ...
    this.port = source.port === undefined ? DEFAULT_PORT : Number(source.port);
    ...
    this.strictProfile = normalizeStrictProfile(source.strictProfile ?? source.strict_profile);
  ...
  _parseExtendedDevice(device, extension) {
    const resolved = resolveExtendedDeviceAndExtension(device, extension || {}, {
      addressProfile: this.addressProfile,
      plcProfile: this.plcProfile,
    });
    return {
      device: requireExplicitPlcProfileForXY(device, this.plcProfile ?? this.addressProfile, resolved.ref),
      extension: resolved.extension,
    };
  }

  _deviceText(device) {
    return deviceToString(device, { addressProfile: this.addressProfile, allowQualifiedOnly: true });
  }

  _ensureProfileFeatureAllowed(featureKey) {
    ensureProfileFeatureAllowed(this.plcProfile, featureKey, this.strictProfile);
  }
  ...
  request(command, subcommand = 0x0000, data = Buffer.alloc(0), options = {}) {
    ...
    validateMonitorRegisterRequest(Number(command), Number(subcommand), data, normalizedSeries, this.plcProfile);
  ...
  async readRandomExt({ wordDevices = [], dwordDevices = [], series, ...requestOptions } = {}) {
    ...
    const words = wordItems.map((entry) => this._parseExtendedDevice(entry.device, entry.extension));
    const dwords = dwordItems.map((entry) => this._parseExtendedDevice(entry.device, entry.extension));
    ...
    const dwordValues = decodeDeviceDwords(response.data.subarray(words.length * 2));
    return {
      word: Object.fromEntries(words.map((entry, index) => [this._deviceText(entry.device), wordValues[index]])),
      dword: Object.fromEntries(dwords.map((entry, index) => [this._deviceText(entry.device), dwordValues[index]])),
    };
  }
  ...
  async writeRandomWordsExt({ wordValues = [], dwordValues = [], series, ...requestOptions } = {}) {
    ...
    const words = wordItems.map((entry) => ({
      ...this._parseExtendedDevice(entry.device, entry.extension),
      value: entry.value,
    }));
    ...
    words.forEach((entry) => {
      this._ensureExtendedProfileFeatureAllowed(entry.device, entry.extension);
      parts.push(encodeResolvedExtendedDeviceSpec(entry.device, { series: normalizedSeries, extension: entry.extension }));
      parts.push(numberToBuffer(Number(entry.value) & 0xffff, 2));
    });
    dwords.forEach((entry) => {
      this._ensureExtendedProfileFeatureAllowed(entry.device, entry.extension);
      parts.push(encodeResolvedExtendedDeviceSpec(entry.device, { series: normalizedSeries, extension: entry.extension }));
      parts.push(numberToBuffer(Number(entry.value) >>> 0, 4));
    });
    await this.request(
      Command.DEVICE_WRITE_RANDOM,
      resolveDeviceSubcommand({ bitUnit: false, series: normalizedSeries, extension: true }),
  ...
  async writeRandomBitsExt({ bitValues = [], series, ...requestOptions } = {}) {
    ...
    bits.forEach((entry) => {
      ...
      parts.push(encodeResolvedExtendedDeviceSpec(entry.device, { series: normalizedSeries, extension: entry.extension }));
      if (normalizedSeries === PLCSeries.IQR) {
        parts.push(numberToBuffer(Boolean(entry.value) ? 1 : 0, 2));
      } else {
        parts.push(Buffer.from([Boolean(entry.value) ? 1 : 0]));
      }
    });
    await this.request(
      Command.DEVICE_WRITE_RANDOM,
...
function normalizeItems(values, addressProfile, plcProfile) {
  if (Array.isArray(values)) {
    return values.map(([device, value]) => [
      requireExplicitPlcProfileForXY(device, plcProfile ?? addressProfile, parseDevice(device, { addressProfile, plcProfile })),
      value,
    ]);
  }
...
function normalizeExtendedDeviceEntries(values, label) {
  return Array.from(values || [], (entry) => {
    if (Array.isArray(entry)) {
      const [device, extension = {}] = entry;
      return { device, extension };
    }
    if (entry && typeof entry === "object" && Object.prototype.hasOwnProperty.call(entry, "device")) {
      return { device: entry.device, extension: entry.extension ?? entry.ext ?? {} };
    }
    if (entry === undefined || entry === null || entry === "") {
      throw new ValueError(`${label} entries must include a device`);
    }
    return { device: entry, extension: {} };
  });
}

function normalizeExtendedValueItems(values, label) {
  if (!Array.isArray(values) && values && typeof values === "object") {
    return Object.entries(values).map(([device, value]) => ({ device, value, extension: {} }));
  }
  return Array.from(values || [], (entry) => {
    if (Array.isArray(entry)) {
      const [device, value, extension = {}] = entry;
      return { device, value, extension };
    }
    if (entry && typeof entry === "object" && Object.prototype.hasOwnProperty.call(entry, "device")) {
      return {
        device: entry.device,
        value: entry.value,
        extension: entry.extension ?? entry.ext ?? {},
      };
    }
    throw new ValueError(`${label} entries must be [device, value, extension] tuples or { device, value, extension } objects`);
  });
}
...
function normalizeBlockItems(values, label, addressProfile, plcProfile) {
  return Array.from(values || [], (item) => {
    if (Array.isArray(item)) {
      ...
      return {
        device: requireExplicitPlcProfileForXY(device, plcProfile ?? addressProfile, parseDevice(device, { addressProfile, plcProfile })),
    ...
    return {
      device: requireExplicitPlcProfileForXY(
        ...
        parseDevice(item.device, { addressProfile, plcProfile })
      ),
