...
function requireExtendedDirectInput(device, operation) {
  if (typeof device !== "string") {
    return;
  }
  const text = device.trim();
  if (!/^(?:U[0-9A-F]+|J[0-9]+)[\\/]/i.test(text)) {
    throw new ValueError(`${operation} requires a qualified U/J address or SlmpExtendedDevice`);
  }
}

function validateDirectReadDevice(ref, points, bitUnit, options = {}) {
  if (!options.allowQualifiedOnly && QUALIFIED_ONLY_CODES.has(ref.code)) {
...
function validateDirectWriteDevice(ref, bitUnit, plcProfile, options = {}) {
  if (!options.allowQualifiedOnly && QUALIFIED_ONLY_CODES.has(ref.code)) {
    throw new ValueError("Direct device access does not support standalone G/HG. Use U-qualified extended access.");
  }
...
function encodeExtendedDirectPayload(entry, points, values, bitUnit, series, maximumLength, label) {
  const specification = prepareResolvedExtendedDeviceSpec(entry.device, {
    series,
    extension: entry.extension,
  });
  ...
  if (values !== null) {
    if (bitUnit) {
      body = packBitValues(values);
    } else {
      body = Buffer.alloc(values.length * 2);
      values.forEach((value, index) => {
        body.writeUInt16LE(requireWireU16(value, `values[${index}]`), index * 2);
      });
    }
  }
  const length = checkedPayloadLength(specification.length + 2, body.length, label);
  validatePreparedPayloadLength(length, maximumLength, label);
  const payload = Buffer.alloc(length);
  let offset = writePreparedExtendedDeviceSpec(payload, 0, specification);
  payload.writeUInt16LE(points, offset);
  offset += 2;
...
class SlmpClient {
  ...
  async readWordsExtended(device, points, options = {}) {
    rejectProfileDerivedOverrides(options, "readWordsExtended");
    this._ensureProfileFeatureAllowed("direct");
    validateDirectAccessPoints(points, false, "readWordsExtended", this.plcProfile, "read");
    requireExtendedDirectInput(device, "readWordsExtended");
    const entry = this._parseExtendedDevice(device);
    requireWordDevice(entry.device, "Extended word read");
    validateDirectReadDevice(entry.device, points, false, { allowQualifiedOnly: true });
    const extendedSeries = selectExtendedEntrySeries([entry], this.plcSeries, "readWordsExtended");
    this._ensureExtendedProfileFeatureAllowed(entry.device, entry.extension);
    validateWireDeviceSpan(
      entry.device.number,
      directConsumedDeviceNumbers(entry.device, points, false),
      extendedSeries,
      "readWordsExtended word",
    );
    const payload = encodeExtendedDirectPayload(
      entry,
      points,
      null,
      false,
    ...
    return this._requestDecoded(
      ...
      options,
      createResponsePhases(
        (response) => validateExactResponseData(response, points * 2, "extended word data length mismatch"),
        (validatedData) => decodeDeviceWords(validatedData),
      ),
    );
  }

  async writeWordsExtended(device, values, options = {}) {
    rejectProfileDerivedOverrides(options, "writeWordsExtended");
    const items = Array.from(values || []);
    this._ensureProfileFeatureAllowed("direct");
    validateDirectAccessPoints(items.length, false, "writeWordsExtended", this.plcProfile, "write");
    requireExtendedDirectInput(device, "writeWordsExtended");
    const entry = this._parseExtendedDevice(device);
    requireWordDevice(entry.device, "Extended word write");
    validateDirectWriteDevice(entry.device, false, this.plcProfile, { allowQualifiedOnly: true });
    const extendedSeries = selectExtendedEntrySeries([entry], this.plcSeries, "writeWordsExtended");
    this._ensureExtendedProfileFeatureAllowed(entry.device, entry.extension);
    validateWireDeviceSpan(
      entry.device.number,
      directConsumedDeviceNumbers(entry.device, items.length, false),
      extendedSeries,
      "writeWordsExtended word",
    );
    const payload = encodeExtendedDirectPayload(
      entry,
      items.length,
  ...
  async readBitsExtended(device, points, options = {}) {
    rejectProfileDerivedOverrides(options, "readBitsExtended");
    this._ensureProfileFeatureAllowed("direct");
    validateDirectAccessPoints(points, true, "readBitsExtended", this.plcProfile, "read");
    requireExtendedDirectInput(device, "readBitsExtended");
    const entry = this._parseExtendedDevice(device);
    validateDirectReadDevice(entry.device, points, true, { allowQualifiedOnly: true });
    const extendedSeries = selectExtendedEntrySeries([entry], this.plcSeries, "readBitsExtended");
    this._ensureExtendedProfileFeatureAllowed(entry.device, entry.extension);
    validateWireDeviceSpan(entry.device.number, points, extendedSeries, "readBitsExtended bit");
    const payload = encodeExtendedDirectPayload(
      entry,
      points,
      null,
    ...
    return this._requestDecoded(
      ...
      options,
      createResponsePhases(
        (response) => validateBitResponseData(response, points),
        (validatedData) => unpackBitValues(validatedData, points),
      ),
    );
  }

  async writeBitsExtended(device, values, options = {}) {
    rejectProfileDerivedOverrides(options, "writeBitsExtended");
    const items = Array.from(values || []);
    this._ensureProfileFeatureAllowed("direct");
    validateDirectAccessPoints(items.length, true, "writeBitsExtended", this.plcProfile, "write");
    requireExtendedDirectInput(device, "writeBitsExtended");
    const entry = this._parseExtendedDevice(device);
    validateDirectWriteDevice(entry.device, true, this.plcProfile, { allowQualifiedOnly: true });
    const extendedSeries = selectExtendedEntrySeries([entry], this.plcSeries, "writeBitsExtended");
    this._ensureExtendedProfileFeatureAllowed(entry.device, entry.extension);
    validateWireDeviceSpan(entry.device.number, items.length, extendedSeries, "writeBitsExtended bit");
    const payload = encodeExtendedDirectPayload(
      entry,
      items.length,
      items,
  ...
  async readRandomExtended({ wordDevices = [], dwordDevices = [], series, ...requestOptions } = {}) {
    if (series !== undefined) throw new ValueError("readRandomExtended does not accept series; it is derived from plcProfile");
    this._ensureProfileFeatureAllowed("random");
    ...
    const extendedSeries = selectExtendedEntrySeries([...words, ...dwords], normalizedSeries, "readRandomExtended");
  ...
  /** @deprecated Use readRandomExtended. */
  async readRandomExt(options = {}) {
    return this.readRandomExtended(options);
  }
  ...
  async registerMonitorDevicesExtended({ wordDevices = [], dwordDevices = [], series, ...requestOptions } = {}) {
    if (series !== undefined) throw new ValueError("registerMonitorDevicesExtended does not accept series; it is derived from plcProfile");
    this._ensureProfileFeatureAllowed("monitor");
    const wordItems = normalizeExtendedDeviceEntries(wordDevices, "wordDevices");
    const dwordItems = normalizeExtendedDeviceEntries(dwordDevices, "dwordDevices");
    if (wordItems.length === 0 && dwordItems.length === 0) {
  ...
  /** @deprecated Use registerMonitorDevicesExtended. */
  async registerMonitorDevicesExt(options = {}) {
    return this.registerMonitorDevicesExtended(options);
  }
  ...
  async writeRandomWordsExtended({ wordValues = [], dwordValues = [], series, ...requestOptions } = {}) {
    if (series !== undefined) throw new ValueError("writeRandomWordsExtended does not accept series; it is derived from plcProfile");
    this._ensureProfileFeatureAllowed("random");
    const normalizedSeries = this.plcSeries;
    ...
    validateNoExtendedRandomWriteOverlap(words, dwords, "writeRandomWordsExtended");
  ...
  /** @deprecated Use writeRandomWordsExtended. */
  async writeRandomWordsExt(options = {}) {
    return this.writeRandomWordsExtended(options);
  }
  ...
  async writeRandomBitsExtended({ bitValues = [], series, ...requestOptions } = {}) {
    if (series !== undefined) throw new ValueError("writeRandomBitsExtended does not accept series; it is derived from plcProfile");
    this._ensureProfileFeatureAllowed("random");
    const normalizedSeries = this.plcSeries;
    ...
    const extendedSeries = selectExtendedEntrySeries(bits, normalizedSeries, "writeRandomBitsExtended");

    for (const entry of bits) {
      this._ensureExtendedProfileFeatureAllowed(entry.device, entry.extension);
    }
    validateBitDeviceSpans(bits.map((entry) => entry.device), extendedSeries, "writeRandomBitsExtended");
    validateNoExtendedBitWriteDuplicates(bits, "writeRandomBitsExtended");
  ...
  /** @deprecated Use writeRandomBitsExtended. */
  async writeRandomBitsExt(options = {}) {
    return this.writeRandomBitsExtended(options);
  }
  ...
  async readLatestSelfDiagnosisErrorCode(options = {}) {
    const values = await this.readDevices("SD0", 1, {
      ...options,
      bitUnit: false,
    });
    return Number(values[0]) & 0xffff;
  }
