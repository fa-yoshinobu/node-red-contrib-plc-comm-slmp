...
class SlmpClient {
  ...
  async readRandomExt({ wordDevices = [], dwordDevices = [], series, ...requestOptions } = {}) {
    ...
    const dwordValues = decodeDeviceDwords(response.data.subarray(words.length * 2));
    return {
      word: Object.fromEntries(words.map((entry, index) => [entry.address, wordValues[index]])),
      dword: Object.fromEntries(dwords.map((entry, index) => [entry.address, dwordValues[index]])),
    };
  }
  ...
  async _requestInternal(command, subcommand, data, options, internalContext = null) {
    ...
    const raw = await this._sendAndReceive(frame, serial, options, internalContext);
  ...
  async _sendAndReceive(frame, serial, options = {}, internalContext = null) {
    if (internalContext !== MANAGED_REMOTE_PASSWORD_COMMAND) {
      await this.connect();
    }
    try {
      return await this._transport.sendAndReceive(frame, serial);
    } catch (error) {
      this._invalidateRemotePasswordState();
      throw error;
    }
