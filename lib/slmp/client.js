...
function validateRandomReadLikeCounts(wordPoints, dwordPoints, series, label, plcProfile, limitKey = "random_read_word") {
  ...
  const fallback = series === PLCSeries.IQR ? 96 : 192;
...
function validateRandomWriteWordCounts(wordPoints, dwordPoints, series, label, plcProfile) {
  const total = wordPoints + dwordPoints;
  if (total < 1) {
    throw new ValueError(`${label} word/dword access points out of range: word=${wordPoints}, dword=${dwordPoints}`);
  }
  const profileLimit = getProfileLimit(plcProfile, "random_write_word");
...
function validateRandomBitWriteCount(points, series, label, plcProfile) {
  const fallback = series === PLCSeries.IQR ? 94 : 188;
  const limit = profileLimitValue(plcProfile, "random_write_bit", fallback);
...
function validateBlockRouteForProfile(plcProfile, commandLabel) {
  ...
  if (["melsec:qcpu", "melsec:qnu"].includes(normalized)) {
...
function isReadOnlyCode(code, plcProfile) {
  ...
  return READ_ONLY_CODES.has(normalizedCode) || isProfileReadOnlyDevice(plcProfile, normalizedCode);
}
...
function validateRandomWriteWordDevices(wordDevices, dwordDevices = [], plcProfile) {
  const readOnlyDevice = [...wordDevices, ...dwordDevices].find((device) => isReadOnlyCode(device.code, plcProfile));
  if (readOnlyDevice) {
    throw new ValueError(`Write Random (0x1402) does not support read-only devices such as ${readOnlyDevice.code}.`);
  }
  if ([...wordDevices, ...dwordDevices].some((device) => QUALIFIED_ONLY_CODES.has(device.code))) {
...
function validateBlockWriteDevices(wordBlocks, bitBlocks, plcProfile) {
  ...
  if (readOnlyBlock) {
    throw new ValueError(`Write Block (0x1406) does not support read-only devices such as ${readOnlyBlock.device.code}.`);
  }
  if ([...wordBlocks, ...bitBlocks].some((block) => QUALIFIED_ONLY_CODES.has(block.device.code))) {
