...
class SlmpClient {
  ...
  _parseDevice(device) {
    const ref = parseDevice(device, { family: this.deviceFamily, plcProfile: this.plcProfile });
    return requireExplicitDeviceFamilyForXY(device, this.plcProfile ?? this.deviceFamily, ref);
  }
  ...
  async readBlock({ wordBlocks = [], bitBlocks = [], series, ...requestOptions } = {}) {
    const normalizedSeries = series ? normalizePlcSeries(series) : this.plcSeries;
    const words = normalizeBlockItems(wordBlocks, "wordBlocks", this.deviceFamily, this.plcProfile);
    const bits = normalizeBlockItems(bitBlocks, "bitBlocks", this.deviceFamily, this.plcProfile);
  ...
  async writeBlock({ wordBlocks = [], bitBlocks = [], series, ...requestOptions } = {}) {
    const normalizedSeries = series ? normalizePlcSeries(series) : this.plcSeries;
    const words = normalizeBlockWriteItems(wordBlocks, "wordBlocks", this.deviceFamily, this.plcProfile);
    const bits = normalizeBlockWriteItems(bitBlocks, "bitBlocks", this.deviceFamily, this.plcProfile);
    if (words.length === 0 && bits.length === 0) {
      throw new ValueError("wordBlocks and bitBlocks must not both be empty");
    }
    if (words.length > 0xff || bits.length > 0xff) {
  ...
  async writeRandomWords({ wordValues = {}, dwordValues = {}, series, ...requestOptions } = {}) {
    const normalizedSeries = series ? normalizePlcSeries(series) : this.plcSeries;
    const wordItems = normalizeItems(wordValues, this.deviceFamily, this.plcProfile);
    const dwordItems = normalizeItems(dwordValues, this.deviceFamily, this.plcProfile);
  ...
  async writeRandomBits({ bitValues = {}, series, ...requestOptions } = {}) {
    const normalizedSeries = series ? normalizePlcSeries(series) : this.plcSeries;
    const items = normalizeItems(bitValues, this.deviceFamily, this.plcProfile);
    if (items.length === 0) {
...
function normalizeItems(values, deviceFamily, plcProfile) {
  if (Array.isArray(values)) {
    return values.map(([device, value]) => [
      requireExplicitDeviceFamilyForXY(device, plcProfile ?? deviceFamily, parseDevice(device, { family: deviceFamily, plcProfile })),
      value,
    ]);
  }
...
function normalizeBlockItems(values, label, deviceFamily, plcProfile) {
  return Array.from(values || [], (item) => {
    if (Array.isArray(item)) {
      const [device, points] = item;
      return {
        device: requireExplicitDeviceFamilyForXY(device, plcProfile ?? deviceFamily, parseDevice(device, { family: deviceFamily, plcProfile })),
    ...
    return {
      device: requireExplicitDeviceFamilyForXY(
        item.device,
        plcProfile ?? deviceFamily,
        parseDevice(item.device, { family: deviceFamily, plcProfile })
      ),
...
function normalizeBlockWriteItems(values, label, deviceFamily, plcProfile) {
  return Array.from(values || [], (item) => {
    if (Array.isArray(item)) {
