...
function directAccessPointLimit(bitUnit, plcProfile) {
  if (!bitUnit) {
    return DIRECT_WORD_POINT_LIMIT;
  }
  return normalizePlcProfile(plcProfile) === "melsec:iq-f" ? DIRECT_IQF_BIT_POINT_LIMIT : DIRECT_BIT_POINT_LIMIT;
}

function validateDirectAccessPoints(points, bitUnit, label, plcProfile) {
  const limit = directAccessPointLimit(bitUnit, plcProfile);
...
function validateBlockRouteForProfile(plcProfile, commandLabel) {
  ...
  if (["melsec:qcpu", "melsec:qnu", "melsec:qnudv"].includes(normalized)) {
    throw new ValueError(`${commandLabel} is not supported for plcProfile '${normalized}'. Use direct or random device commands.`);
  }
}
...
function validateDirectWriteDevice(ref, bitUnit) {
  ...
  if (READ_ONLY_CODES.has(ref.code)) {
    throw new ValueError(`${ref.code} is read-only and cannot be written.`);
  }
...
function validateRandomReadDevices(wordDevices, dwordDevices) {
  if ([...wordDevices, ...dwordDevices].some((device) => QUALIFIED_ONLY_CODES.has(device.code))) {
...
function validateRandomWriteWordDevices(wordDevices, dwordDevices = []) {
  if ([...wordDevices, ...dwordDevices].some((device) => READ_ONLY_CODES.has(device.code))) {
    throw new ValueError("Write Random (0x1402) does not support read-only devices such as S.");
  }
  if ([...wordDevices, ...dwordDevices].some((device) => QUALIFIED_ONLY_CODES.has(device.code))) {
...
function validateRandomWriteBitDevices(bitDevices) {
  if (bitDevices.some((device) => READ_ONLY_CODES.has(device.code))) {
    throw new ValueError("Write Random (0x1402) does not support read-only devices such as S.");
  }
...
function validateBlockWriteDevices(wordBlocks, bitBlocks) {
  if ([...wordBlocks, ...bitBlocks].some((block) => READ_ONLY_CODES.has(block.device.code))) {
    throw new ValueError("Write Block (0x1406) does not support read-only devices such as S.");
  }
  if ([...wordBlocks, ...bitBlocks].some((block) => QUALIFIED_ONLY_CODES.has(block.device.code))) {
...
class SlmpClient {
  ...
  _deviceText(device) {
    return deviceToString(device, { addressProfile: this.addressProfile });
  }
  ...
  async readDevices(device, points, options = {}) {
    ...
    validateDirectAccessPoints(points, bitUnit, "readDevices", this.plcProfile);
  ...
  async writeDevices(device, values, options = {}) {
    ...
    validateDirectAccessPoints(items.length, bitUnit, "writeDevices", this.plcProfile);
    const ref = this._parseDevice(device);
  ...
  async writeRandomBits({ bitValues = {}, series, ...requestOptions } = {}) {
    ...
    validateRandomWriteBitDevices(items.map(([device]) => device));
